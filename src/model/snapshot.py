...
class SnapshotModelCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        #     print('now we begin to train our embeding layers')
        #     self.model.summary()
...
class SnapshotCallbackBuilder:
    ...
    def get_callbacks(self, model_save_place='./', model_prefix='Model'):
        """
        ...
        """
        if not os.path.exists(model_save_place):
            os.makedirs(model_save_place)

        callback_list = [
                         callbacks.LearningRateScheduler(schedule=self._cosine_anneal_schedule),
                         SnapshotModelCheckpoint(self.T, self.M, fn_prefix='%s/%s' % (model_save_place, model_prefix))]
    ...
    def _cosine_anneal_schedule(self, t):
        ...
        alpha = float(self.alpha_zero / 2 * cos_out)
